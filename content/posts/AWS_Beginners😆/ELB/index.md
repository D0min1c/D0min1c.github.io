...
3가지 종류의 로드밸런스를 지원하고 최근 1개가 추가되어 총 4가지의 로드밸런서 타입을 지원하지만 앞선 3개의 타입에 대해서 우선 알아보겠습니다.
...
tcp sync를 먼저 백엔드서버에 요청하여 세션을 미리 맺어 놓은 뒤, 실제 트래픽 발생 시, 해당 세션을 재사용하여 latency를 줄입니다.

- ALB (Application Load Balancer)
7계층 L7 로드밸런서로 Ip, port, Packet based의 라우팅 처리를 합니다.
SG를 적용하여 클라이언트의 요청에 대해 로드밸런서에서 필터링할 수 있습니다.
...
4계층 L4 로드밸런서로 IP, port based의 라우팅 처리를 합니다.
라우팅 알고리즘
...
따라서 TCP 트래픽을 로드밸런싱 할 때나 외부에 노출될 고정 IP가 필요한 경우 사용합니다.

ALB – HTTP 및 HTTPS 트래픽의 고급 로드 밸런싱에 이상적인 ALB는 마이크로서비스 및 컨테이너 기반 응용 프로그램을 포함한 최신 응용 프로그램 아키텍처를 지원하는 고급 요청 라우팅을 제공합니다.
따라서 Http/Https 트래픽의 고급요청 라우팅이 필요할 때 사용합니다.

CLB - 이제는 EC2-Classic을 지원하지 않을 뿐더러, (남아있는 EC2-Classic Platform도 현재 메인터넌스 일정에 사라질 예정) NLB,ALB에서 동일한 기능 대비 좋은 성능과 옵션을 제공하기에 쓸 이유가 없습니다.
